 ...
 * @param lambda    Value to be used for computing the fitness value.
 */
vector<int> localSearch(const vector<vector<float>> &X, const vector<triplet> &ML, const vector<triplet> &CL, int k, double lambda, double &fitness)
{
...
vector<int> BMB(const vector<vector<float>> &X, const vector<triplet> &ML, const vector<triplet> &CL, int k, double lambda)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        S = localSearch(X, ML, CL, k, lambda, actFitness);
...
int main(int argc, char * argv[])
{
    ...
    vector<int> result = BMB(X, ML, CL, numClusters, lambda);
    timeAfter = clock();

    // Print the data
    output("BMB", result, X, ML, CL, numClusters, lambda, (double)(timeAfter-timeBefore) / CLOCKS_PER_SEC, stoi(argv[6]));
